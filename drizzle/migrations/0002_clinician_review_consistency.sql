-- Forward-only correction: serialize authority revocation with clinical review,
-- authorize durable replays, and bind one exact grant/review across all receipts.
-- No existing grants or patient records are changed.
...
begin
  ...
  -- Acquire the session lock before testing expiry, so waiting for another
  -- writer cannot make a stale transaction-start timestamp authorize a write.
  ...
  if prior.id is null and p_disposition = 'permit_resumption' then
    -- The edge binds this exact grant into its hashed state; never substitute a
    -- newer grant silently. FOR SHARE conflicts with revocation's UPDATE lock.
    ...
    -- Reading a replay also requires current authority for the same patient.
    select * into authority from public.clinician_patient_authorizations
  ...
  -- Check again after row-lock acquisition with wall-clock time. A concurrent
  -- revocation either finishes first and is denied here, or waits for this
  -- transaction to finish. An expired grant cannot authorize a queued review.
