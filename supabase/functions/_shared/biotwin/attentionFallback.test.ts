...
Deno.test("GOLDEN SUBJECT-01: attention question yields a substantive packet-grounded answer", () => {
...
Deno.test("GOLDEN SUBJECT-01: still holds with medication + pgx + decision_grade holds active", () => {
...
Deno.test("version stamps are current", async () => {
  ...
  assertEquals(BIOTWIN_VALIDATOR_VERSION, "1.3.0");
  assertEquals(RUNTIME_VERSION, "r0.1.6");
...
const SUBJ01_APOB_TITLE =
  "Measured atherogenic particle burden (ApoB 124 mg/dL on 2026-02-20 and 118 mg/dL on 2026-04-02 statin OFF; LDL-P 1574 nmol/L) of unquantified etiology";
const SUBJ01_TOBACCO_TITLE =
...
const SUBJ01_VASCULAR_TITLE =
  "Vascular-injury-associated protein hypothesis (abundance signals of unmeasured activity)";
const SUBJ01_IRON_TITLE =
  "One abnormal iron study of unestablished persistence and etiology (serum iron 266 mcg/dL, saturation 78%, ferritin 61 within range)";

function syntheticDriverPacket() {
  return buildBiotwinPacket(report, [
    row({ source_id: "drv-apob", statement_kind: "driver", truth_status: "confirmed", title: SUBJ01_APOB_TITLE, ordinal: 1 }),
    row({ source_id: "drv-tobacco", statement_kind: "driver", truth_status: "confirmed", title: SUBJ01_TOBACCO_TITLE, ordinal: 2 }),
    row({ source_id: "drv-vascular", statement_kind: "driver", truth_status: "confirmed", title: SUBJ01_VASCULAR_TITLE, ordinal: 3 }),
    row({ source_id: "drv-iron", statement_kind: "driver", truth_status: "confirmed", title: SUBJ01_IRON_TITLE, ordinal: 4 }),
  ]);
}
...
Deno.test("non-confirmed truth_status is always hypothesis", () => {
  const p = buildBiotwinPacket(report, [
    ...
    row({ source_id: "drv-m", statement_kind: "driver", truth_status: "confirmed", title: SUBJ01_TOBACCO_TITLE, ordinal: 2 }),
  ]);
