...
export function convertSupplementWithDose(intervention: Intervention): CoreSafeAction {
  ...
  return {
    ...
    what: intervention.core_title ?? `Discuss ${supplementName} with your clinician.`,
    how: intervention.core_observation ?? `Bring your latest lab results and the rationale below to the conversation. Your clinician can determine the right approach for you.`,
    rationale: intervention.core_rationale ?? stripDoseFromRationale(intervention),
    doctor_question: intervention.core_clinician_question ?? `What ${supplementName} approach is appropriate given my current levels and any other factors you're considering?`,
...
export function convertMedicationChange(intervention: Intervention): CoreSafeAction {
  return {
    ...
    source_intervention_id: intervention.id,
    policy_class: "doctor_question",
    what: intervention.core_title ?? `Discuss your current medication with your clinician.`,
    how: intervention.core_observation ?? `Medication changes need a clinician who knows your full history. Bring this rationale and your symptom log to the conversation.`,
    rationale: intervention.core_rationale ?? stripDoseFromRationale(intervention),
    doctor_question: intervention.core_clinician_question ?? `Given my current data, would a different medication approach be worth considering for me?`,
    coordinates: intervention.coordinates,
    sequence_priority: intervention.sequence_priority,
  };
}
...
export function convertTitration(intervention: Intervention): CoreSafeAction {
  return {
    ...
    source_intervention_id: intervention.id,
    policy_class: "doctor_question",
    what: intervention.core_title ?? `Discuss whether your current approach is still right for you.`,
    how: intervention.core_observation ?? `Bring your tracking data — symptoms, labs, and how you're feeling — to your clinician. Adjustments are a clinical decision.`,
    rationale: intervention.core_rationale ?? stripDoseFromRationale(intervention),
    doctor_question: intervention.core_clinician_question ?? `Based on my recent data, should we revisit my current approach?`,
    coordinates: intervention.coordinates,
    sequence_priority: intervention.sequence_priority,
  };
}
...
export function convertIndividualizedProtocol(intervention: Intervention): CoreSafeAction {
  return {
    ...
    source_intervention_id: intervention.id,
    policy_class: "doctor_question",
    what: intervention.core_title ?? `Discuss a coordinated approach with your clinician.`,
    how: intervention.core_observation ?? `The pattern in your data suggests several connected signals worth addressing together. Bring this rationale to your clinician.`,
    rationale: intervention.core_rationale ?? stripDoseFromRationale(intervention),
    doctor_question: intervention.core_clinician_question ?? `Given the pattern in my data, would a coordinated plan addressing these signals together be appropriate?`,
    coordinates: intervention.coordinates,
    sequence_priority: intervention.sequence_priority,
  };
}
