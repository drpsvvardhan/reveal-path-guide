...
serve(async (req) => {
  ...
  try {
    const { messages, patientContext, documents } = await req.json();

    const LOVABLE_API_KEY = Deno.env.get("LOVABLE_API_KEY");
    if (!LOVABLE_API_KEY) throw new Error("LOVABLE_API_KEY is not configured");

    // Build system prompt with patient context and documents
    let systemPrompt = `You are a compassionate, knowledgeable patient care reasoning assistant for the Vizzhy PatientOS platform. You help patients understand their health data, lab results, and care plans in clear, warm, non-clinical language.
