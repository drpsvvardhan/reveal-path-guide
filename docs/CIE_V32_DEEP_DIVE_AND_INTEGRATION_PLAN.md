# CIE v3.2/v3.3 — Deep-Dive Analysis and Patient Reveal Integration Plan

## Revision 4 — the v3.3 Canonical Set changes the build strategy

`CIE_v3_3_Canonical_Set.zip` is not another treatise revision: it is the **first code-backed release** — release manifest, SHA256SUMS, the v3.2 treatise unchanged as doctrine, a 48-section executable specification, a 16-entry hash-bound delta manifest, `constitutional-laws.yaml` with stable law IDs, a traceability matrix (204 normative contracts: 86 directly covered, 118 partial/production-dependent, 0 orphans), and a **reference kernel**: dependency-free TypeScript (~1,700 source lines), a `canonical-registry.json` machine source of truth with a generation pipeline (TS + JSON Schema + OpenAPI), 12 golden fixtures, and a 44-test conformance suite.

**Independently verified from the clean unpack:** all 99 SHA256SUMS check; the bundled doctrine docx is byte-identical to the v3.2 treatise analyzed in Revision 3; `generate-registry --check`, `generate-contracts --check`, `generate-golden-manifest --check`, and the build all pass; the full test suite passes **44/44** on re-run (their packaging used Node 24; re-verified here under Node 22 with type stripping). The release's own claim is deliberately bounded: `conformant_to_narrowed_profile`, `not-clinically-validated`, `not-production-authorized` — no persistence, no HTTP server, no authenticated transport/ABAC controls, and no machine-readable migration source for all 359 authored records. Those exclusions are stated in the conformance report rather than papered over, which is itself conformant behavior.

**The headline normative delta (CIE33-DELTA-002/003): `ObservableRef`** — a closed two-variant union, `registered` (resolves a versioned canonical concept) vs `session_local` (an observable first encountered in open testimony, preserving `sourceLabel`, introducing witness, scope, and mapping status). A session-local observable can be characterized by generic registered operations but can never satisfy a sentinel, become a coded clinical fact, trigger external action, aggregate cross-person, or be promoted by model output alone — promotion is an append-only mapping assertion. The formula: **open at capture → governed in composition → closed at commit.** The `unseen-kashayam` golden fixture proves it: exact source text preserved, `mustNotGuess: [ingredient, dose, diagnosis, causation]`.

**Plan impact — Stages 1–4 change from "author" to "adapt":**

1. **M1 (registry):** do not author `canonical.ts` — adopt `reference-kernel/registry/canonical-registry.json` and its generation pipeline verbatim, and *extend the generator* with one more emitter: Postgres CHECK fragments for the `cie3_*` migrations (the kernel emits TS/JSON Schema/OpenAPI; SQL is our addition). The registry hash pins in `cie3_sessions` come from this artifact.
2. **M2–M3 (contracts, referee, compilers):** vendor `reference-kernel/src/` into `supabase/functions/_shared/cie3/kernel/` (it is dependency-free TS, so it runs under both Deno edge functions and vitest via the existing `@shared` alias — the same dual-runtime property we planned to build by hand). Patient Reveal's work narrows to exactly the 118 production-dependent contracts: the append-only persistence of the §28 event model onto the `cie3_*` tables, the §31 Command API as edge functions, ABAC/consent wiring, and the client.
3. **Conformance in our CI:** the 31 required tests + golden fixtures run as-is in the repo's CI (alongside the existing golden-fixture workflows); Patient Reveal adds its own fixtures (factory import, terrain staleness, queue integration) on top. Un-narrowing the conformance claim — persistence, transport, security, the 359-record migration — **is** the Patient Reveal integration, stage by stage.
4. **`session_local` promotion maps onto existing machinery:** the repo's RAE concept-assignment layer (`concept_assignment_witnesses`, admission states, deterministic signals) is nearly the promotion pathway CIE33-DELTA-003 requires — an append-only mapping assertion with governed review. Wiring `ObservableRef` promotion through RAE instead of building a parallel path is the single biggest reuse win in the whole integration.
5. **Authority order adopted as repo policy** for this workstream: v3.2 constitution → v3.3 delta manifest → v3.3 contract text + canonical registry → generated schemas → kernel behavior → prose. A conflict between levels is a release defect.

Everything below (Revisions 1–3) remains the record of how the target was derived; milestone content stands except where this addendum redirects M1–M4 from authoring to adaptation.
